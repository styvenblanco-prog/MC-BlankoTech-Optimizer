...
class OptimizationEngine {
    ...
    /**
     * Solicita al sistema establecer el modo de alto rendimiento para el paquete de Minecraft.
    ...
    fun acquireWakeLock(context: Context, durationMs: Long = 10 * 60 * 1000L) {
        try {
            ...
            Log.e("OptimizationEngine", "Error adquiriendo WakeLock", e)
        }
    }
    ...
    fun releaseWakeLock() {
        try {
            if (wakeLock?.isHeld == true) {
                wakeLock?.release()
                Log.d("OptimizationEngine", "WakeLock liberado")
            }
        } catch (e: Exception) {
            Log.e("OptimizationEngine", "Error liberando WakeLock", e)
        }
    }
}
